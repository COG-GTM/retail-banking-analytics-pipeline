            ...
            COUNT(DISTINCT CASE
                WHEN t.transaction_date >= current_date - 30
                 AND t.merchant_name NOT IN (
                     SELECT t2.merchant_name
                     FROM txn_processing.transactions t2
...
def _phase3b_txn_analytics(con: duckdb.DuckDBPyConnection) -> None:
    ...
    cust = df.groupby("customer_id").agg(
        ...
        top_spend_category=("top_merchant_category", "max"),
    ).reset_index()
    ...
    # Percentile ranking (mirrors SAS PROC RANK groups=100 -> integer groups 0..99)
    _n = int(cust["total_debit_amt"].notna().sum())
    _rank_asc = cust["total_debit_amt"].rank(method="average", ascending=True)
    cust["spend_percentile"] = np.floor(_rank_asc * 100 / (_n + 1))
    ...
    out_cols = ["customer_id", "reporting_period", "total_accounts", "active_accounts",
                ...
                "revenue_contribution", "anomaly_flag", "model_version", "effective_date",
                "load_ts"]
    con.register("_txn_df", cust[out_cols])
