...
def _sas_customer_segments(con: duckdb.DuckDBPyConnection) -> None:
    ...
    # Feature engineering (sas DATA step WORK.CUST_FEATURES)
    df["product_breadth"] = (
    ...
    standardised = StandardScaler().fit_transform(feats)  # PROC STDIZE method=std

    k = min(5, len(df))
    km = KMeans(n_clusters=k, max_iter=50, n_init=10, random_state=SEED)  # PROC FASTCLUS k=5
    df["segment_id"] = km.fit_predict(standardised)

    # Order clusters by avg log-balance and label by rank (sas WORK.SEGMENT_LABELS)
    profile = df.groupby("segment_id")["log_balance"].mean().sort_values(ascending=False)
...
def _sas_txn_analytics(con: duckdb.DuckDBPyConnection) -> None:
    ...
    # PROC RANK groups=100 -> spend percentile (0-100)
    df["spend_percentile"] = (df["total_debit_amt"].rank(pct=True) * 100).round(2)
...
def _sas_risk_scoring(con: duckdb.DuckDBPyConnection) -> None:
    ...
    # Feature prep (sas WORK.RISK_FEATURES)
    ...
    # PROC LOGISTIC: model PROB_DEFAULT. Falls back to base rate when the
    # late-payment proxy yields a single-class target (see docs migration note).
    if y.nunique() < 2:
    ...
    # Composite scoring components (each clipped to 0-100)
    def clip(s):
...
def _sas_data_products(con: duckdb.DuckDBPyConnection) -> None:
    ...
    # Defaults for customers missing in an upstream product (sas MERGE IN= handling)
