...
-- pipeline. They are the "contract" with downstream consumers (dashboards,
-- reports, APIs).
--
-- Usage: snowsql -f ddl/snowflake/03_data_product_tables.sql -D env=DEV
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- DATA_PRODUCTS.CUSTOMER_SEGMENTS
...
COMMENT = 'Customer segmentation data product (migrated from DATA_PRODUCTS_DB.CUSTOMER_SEGMENTS)';

-- -----------------------------------------------------------------------------
-- DATA_PRODUCTS.TRANSACTION_ANALYTICS
-- Per-customer transaction behaviour summary with trend indicators.
-- Teradata PARTITION BY COLUMN(REPORTING_PERIOD) is replaced by a Snowflake
-- clustering key on REPORTING_PERIOD, which is the standard consumer filter.
-- Refresh: Daily
...
COMMENT = 'Transaction analytics data product (migrated from DATA_PRODUCTS_DB.TRANSACTION_ANALYTICS)';

-- -----------------------------------------------------------------------------
-- DATA_PRODUCTS.CUSTOMER_RISK_SCORES
...
COMMENT = 'Customer risk score data product (migrated from DATA_PRODUCTS_DB.CUSTOMER_RISK_SCORES)';
...
COMMENT = 'Golden record customer master profile (migrated from DATA_PRODUCTS_DB.CUSTOMER_MASTER_PROFILE)';
