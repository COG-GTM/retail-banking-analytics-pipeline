...
-- =============================================================================
-- These tables are rebuilt on every pipeline run, so CREATE OR REPLACE TABLE
-- preserves the Teradata DROP/CREATE semantics while staying re-runnable.
--
...
-- -----------------------------------------------------------------------------
-- ETL_STAGING.STG_CUSTOMER_360  (was ETL_STAGING_DB.STG_CUSTOMER_360)
-- Denormalized customer view joining customer, account, and address data.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE TABLE ETL_STAGING.STG_CUSTOMER_360
(
...
-- -----------------------------------------------------------------------------
-- ETL_STAGING.STG_TXN_SUMMARY  (was ETL_STAGING_DB.STG_TXN_SUMMARY)
-- Aggregated transaction metrics per customer over configurable lookback.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE TABLE ETL_STAGING.STG_TXN_SUMMARY
(
    CUSTOMER_ID             NUMBER(19,0)    NOT NULL,
...
-- -----------------------------------------------------------------------------
-- ETL_STAGING.STG_RISK_FACTORS  (was ETL_STAGING_DB.STG_RISK_FACTORS)
-- Pre-computed risk indicator features per customer for scoring models.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE TABLE ETL_STAGING.STG_RISK_FACTORS
(
    CUSTOMER_ID             NUMBER(19,0)    NOT NULL,
...
-- Teradata COLLECT STATISTICS on the CUSTOMER_ID / (CUSTOMER_ID, ACCOUNT_ID)
-- columns has no Snowflake equivalent: statistics are maintained automatically
-- per micro-partition by the service, so those statements are dropped.
