-- =============================================================================
-- Staging Table DDL (Delta) - ETL Intermediate Tables (Silver)
-- =============================================================================
-- Delta port of ddl/01_staging_tables.sql. Populated by the PySpark staging
-- jobs (ports of bteq/*.bteq) and consumed by the gold jobs (ports of sas/*.sas).
--   Teradata ETL_STAGING_DB -> ${catalog}.${schema_stg}
-- Legacy COLLECT STATISTICS statements are dropped (Delta collects stats
-- automatically). Writes are idempotent (overwrite/MERGE), replacing the
-- legacy DROP/CREATE-per-run pattern.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Denormalized customer view joining customer, account, and address data.
-- Populated by: jobs/stg_customer_360 (port of bteq/01_stg_customer_360.bteq)
-- -----------------------------------------------------------------------------
...
-- Populated by: jobs/stg_txn_summary (port of bteq/02_stg_txn_summary.bteq)
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS ${catalog}.${schema_stg}.stg_txn_summary (
    customer_id             BIGINT      NOT NULL,
...
-- Populated by: jobs/stg_risk_factors (port of bteq/03_stg_risk_factors.bteq)
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS ${catalog}.${schema_stg}.stg_risk_factors (
    customer_id             BIGINT      NOT NULL,
