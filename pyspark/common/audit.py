"""ETL audit trail.

MINIMAL SHARED STUB (owned by Ticket 3 — shared utilities). Replaces the SAS
``%log_step`` macro + ``ETL_RUN_LOG`` table. ``log_step`` appends one row to the
Delta ``etl_staging.etl_run_log`` table AND emits a structured JSON log line
(no bare ``print``).
"""

from __future__ import annotations

import json
import logging
from datetime import datetime, timezone

from pyspark.sql import SparkSession
from pyspark.sql.types import (
    LongType,
    StringType,
    StructField,
    StructType,
    TimestampType,
)

from common.config import Config

_LOGGER = logging.getLogger("retail_banking.audit")
if not _LOGGER.handlers:
    _handler = logging.StreamHandler()
    _handler.setFormatter(logging.Formatter("%(message)s"))
    _LOGGER.addHandler(_handler)
    _LOGGER.setLevel(logging.INFO)

_AUDIT_SCHEMA = StructType(
    [
        StructField("run_id", StringType(), True),
        StructField("job_name", StringType(), True),
        StructField("step", StringType(), True),
        StructField("status", StringType(), True),
        StructField("row_count", LongType(), True),
        StructField("message", StringType(), True),
        StructField("log_ts", TimestampType(), True),
    ]
)


def _audit_table(cfg: Config) -> str:
    return cfg.table(cfg.schema_stg, "etl_run_log")


def init_audit(spark: SparkSession, cfg: Config) -> None:
    """Create the ``etl_staging.etl_run_log`` Delta table if it does not exist."""
    spark.sql(f"CREATE DATABASE IF NOT EXISTS {cfg.catalog}.{cfg.schema_stg}")
    table = _audit_table(cfg)
    (
        spark.createDataFrame([], _AUDIT_SCHEMA)
        .write.format("delta")
        .mode("ignore")
        .saveAsTable(table)
    )


def log_step(
    spark: SparkSession,
    cfg: Config,
    run_id: str,
    job_name: str,
    step: str,
    status: str,
    row_count: int | None = None,
    message: str | None = None,
) -> None:
    """Append one audit row and emit a structured JSON log line."""
    log_ts = datetime.now(timezone.utc)
    _LOGGER.info(
        json.dumps(
            {
                "run_id": run_id,
                "job_name": job_name,
                "step": step,
                "status": status,
                "row_count": row_count,
                "message": message,
                "log_ts": log_ts.isoformat(),
            }
        )
    )
    row = [(run_id, job_name, step, status, row_count, message, log_ts)]
    (
        spark.createDataFrame(row, _AUDIT_SCHEMA)
        .write.format("delta")
        .mode("append")
        .saveAsTable(_audit_table(cfg))
    )
