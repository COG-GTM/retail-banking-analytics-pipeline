"""ETL audit trail.

Minimal stub matching the shared audit contract so this ticket's PR is
self-contained. Replaces the SAS ``%log_step`` macro + ``ETL_RUN_LOG`` table.
"""
from __future__ import annotations

import json
import logging
from datetime import datetime, timezone

from pyspark.sql import Row, SparkSession
from pyspark.sql.types import (
    LongType,
    StringType,
    StructField,
    StructType,
    TimestampType,
)

from common.config import Config

_LOGGER = logging.getLogger("retail_banking_analytics.audit")
if not _LOGGER.handlers:
    _handler = logging.StreamHandler()
    _handler.setFormatter(logging.Formatter("%(message)s"))
    _LOGGER.addHandler(_handler)
    _LOGGER.setLevel(logging.INFO)

_AUDIT_SCHEMA = StructType(
    [
        StructField("run_id", StringType(), False),
        StructField("job_name", StringType(), False),
        StructField("step", StringType(), False),
        StructField("status", StringType(), False),
        StructField("row_count", LongType(), True),
        StructField("message", StringType(), True),
        StructField("log_ts", TimestampType(), False),
    ]
)


def _audit_table(cfg: Config) -> str:
    return cfg.table(cfg.schema_stg, "etl_run_log")


def init_audit(spark: SparkSession, cfg: Config) -> None:
    """Create the ``etl_staging.etl_run_log`` Delta table if it does not exist."""
    spark.sql(f"CREATE SCHEMA IF NOT EXISTS {cfg.catalog}.{cfg.schema_stg}")
    cols = (
        "run_id STRING, job_name STRING, step STRING, status STRING, "
        "row_count BIGINT, message STRING, log_ts TIMESTAMP"
    )
    spark.sql(
        f"CREATE TABLE IF NOT EXISTS {_audit_table(cfg)} ({cols}) USING DELTA"
    )


def log_step(
    spark: SparkSession,
    cfg: Config,
    run_id: str,
    job_name: str,
    step: str,
    status: str,
    row_count: int | None = None,
    message: str | None = None,
) -> None:
    """Append one audit row and emit a structured JSON log line."""
    now = datetime.now(timezone.utc)
    payload = {
        "run_id": run_id,
        "job_name": job_name,
        "step": step,
        "status": status,
        "row_count": row_count,
        "message": message,
        "log_ts": now.isoformat(),
    }
    _LOGGER.info(json.dumps(payload))

    row = Row(
        run_id=run_id,
        job_name=job_name,
        step=step,
        status=status,
        row_count=int(row_count) if row_count is not None else None,
        message=message,
        log_ts=now,
    )
    df = spark.createDataFrame([row], schema=_AUDIT_SCHEMA)
    try:
        df.write.format("delta").mode("append").saveAsTable(_audit_table(cfg))
    except Exception:
        # Audit table not provisioned in this environment; JSON log line stands.
        pass


__all__ = ["init_audit", "log_step"]
