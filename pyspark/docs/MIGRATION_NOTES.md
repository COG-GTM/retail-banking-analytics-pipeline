...
| `CREDIT_UTIL_RATIO` is `DECIMAL(5,4)` per the DDL, so a utilisation of 10 or more aborts the job under Spark's ANSI mode. Preserved, not widened — Teradata raises a numeric overflow on the same `CAST(... AS DECIMAL(5,4))`. | The contract, not the port, bounds the value; the performance tier's synthetic card balances are drawn against their limit so they stay inside it, as real card data does. |
