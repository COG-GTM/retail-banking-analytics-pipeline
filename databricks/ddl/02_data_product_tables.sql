-- =============================================================================
-- Data Product Table DDL - Certified Analytical Outputs (Databricks / Delta)
-- =============================================================================
-- Migrated from ddl/02_data_product_tables.sql (Teradata, DATA_PRODUCTS_DB).
--
-- These are the final, consumable data products produced by the analytics
-- notebooks. They are the "contract" with downstream consumers (dashboards,
-- reports, APIs). The analytics notebooks write them with
-- df.write.mode("overwrite").saveAsTable(...), so these definitions document
-- the schema and pre-create the schema / tables on first deploy.
--
-- Migration notes mirror the other DDL files:
--   MULTISET/NO FALLBACK -> USING DELTA; PRIMARY INDEX -> CLUSTER BY;
--   VARCHAR/CHAR -> STRING; TIMESTAMP(6) -> TIMESTAMP; FORMAT dropped;
--   COLLECT STATISTICS dropped; Teradata column-partitioning -> CLUSTER BY.
-- =============================================================================
...
-- (Teradata partitioned by REPORTING_PERIOD -> CLUSTER BY here.)
-- -----------------------------------------------------------------------------
...
USING DELTA
CLUSTER BY (customer_id, reporting_period);
