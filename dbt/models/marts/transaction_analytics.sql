...
select
    ...
    cast(
        trim(cast(extract(year from current_date) as integer)) || '-'
        || case when extract(month from current_date) < 10 then '0' else '' end
        || trim(cast(extract(month from current_date) as integer))
        as varchar(7)
    )                                                        as reporting_period,
    ...
    cast('{{ var("model_version_txn", "TXN_V2.1") }}' as varchar(20)) as model_version,
