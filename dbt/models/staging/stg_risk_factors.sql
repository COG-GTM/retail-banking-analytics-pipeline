...
-- ADD_MONTHS and QUALIFY are supported natively by Databricks SQL. Teradata
-- date - N arithmetic is rewritten as DATE_SUB, and the correlated NOT IN
-- subquery (new-merchant detection) becomes a left join against a
-- prior_merchants CTE, since Databricks does not support correlated
-- subqueries inside aggregate expressions.
...
prior_merchants as (
    ...
    -- Merchants each account had already used before the 30-day window.
    select distinct
        t2.account_id,
        t2.merchant_name
    from {{ source('txn_processing', 'transactions') }} t2
    where t2.transaction_date < date_sub(current_date, 30)
